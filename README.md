...
<p align="center">
  ...
  <a href="#-model-context-protocol-mcp-tools">MCP Tools</a> •
...
> 💡 **New to sv-memory?** Check out the step-by-step [Getting Started & Installation Guide](documentation/getting_started_guide.md) ([Español](documentation/getting_started_guide_ES.md)), the [Codebase Guide](documentation/CODEBASE-GUIDE.md) for a tour of the key data flows, or read the [Español README](README_ES.md).
...
| ⚖️ **Governance**    | **Spec-Driven Decisions**  | Native propose → validate → commit cycle: `sv_propose_spec`/`sv_validate_decision`/`sv_commit_spec` pre-flight proposals against rules and invariants before code is written, with OpenSpec-style delta requirements (ADDED/MODIFIED/REMOVED/RENAMED + RFC 2119 + scenarios) merged into a durable per-capability state. |
...
> **After updating, re-wire your agents.** The update only replaces the binary — it does
> not refresh your MCP allow-lists or injected protocol. Run `sv-memory setup --all` (or
> `setup <agent>` per agent) so newly added MCP tools are granted and the protocol/skill
> text matches the new binary, then restart your assistant. See
> [AGENT-SETUP.md](documentation/AGENT-SETUP.md#updating-sv-memory-post-update).
...
> **Note (macOS Apple Silicon):** Always use `mv` rather than `cp` when installing the binary. Overwriting an existing binary in place triggers a macOS kernel SIGKILL with "Code Signature Invalid" ([golang/go#63997](https://github.com/golang/go/issues/63997)).
...
```bash
sv-memory permissions status --platform antigravity   # Granted: 43 / 43
...
| `sv-memory specs export/import/list/archive/capabilities` | **Spec Mirror** | Manages the human-readable Markdown projection of spec changes and capability state under `.sv-memory/specs/` (auto-written by sync; import reconciles human edits back into the authoritative store; capabilities lists the current requirement state). |
...
| `sv-memory capture prompt "<text>"` | **Memory**     | Captures the user's prompt as a local observation attached to the active session (CLI parity with `sv_mem_capture_prompt`, for hooks/plugins). |
| `sv-memory session start`          | **Session**     | Starts a coding session and prints the Auto-Boot context bundle (CLI parity with `sv_mem_session_start`, for hooks/plugins). |
| `sv-memory session summary <id>`   | **Session**     | Saves a structured summary (goal/discoveries/accomplished/next-steps/files) for a session. |
| `sv-memory session end`            | **Session**     | Ends the active session (or `--session-id`) with an optional summary, mirroring `sv_mem_session_end`. |
| `sv-memory session active`         | **Session**     | Prints the active session ID or `none`.                                                 |
...
- **`sv_mem_search`**: FTS5 search with **BM25 ranking**, category/path filters, and **match_mode** (`all` / `any`). `graph_boost` (default on) expands a `path` search to the whole graph community, annotating community rows with `[graph]`.
...
- **`sv_mem_stats`**: Aggregate memory statistics and per-category breakdowns, plus the current active project (ID, name, path) and the **session token ledger** (estimated tokens injected since `sv_mem_session_start` vs the `max_response_tokens` budget).
...
```bash
# Show the 43 tools with descriptions
sv-memory permissions list

# Grant all 43 tools to Antigravity CLI (dry-run first to preview)
...
In the `sv-memory configure` wizard, **Phase 4** lists the 43 tools for you to choose

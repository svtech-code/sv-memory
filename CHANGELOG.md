...
- **Model-agnostic OpenCode plugin automation**: the strict OpenCode plugin now auto-starts the session and injects the Auto-Boot Context Bundle on the first user request, captures every user prompt (`chat.message` → `sv-memory capture prompt`), re-orients the model with active specs when the model changes mid-session (`experimental.chat.messages.transform`), emits a corrective spec-flow nudge after an edit that skipped `sv_spec_list`/`sv_propose_spec` (`tool.execute.after` + transform), and reminds the agent to save the session summary before opencode compacts (`experimental.session.compacting`). All hooks are fail-open and fire once, so sv-memory behaves the same for any model. Protocol + skill note that the session is auto-managed on OpenCode.
- **Deterministic Claude Code lifecycle hooks**: `SessionStart` now auto-starts the sv-memory session and injects the Auto-Boot Context Bundle as `additionalContext` (fail-open), a new `UserPromptSubmit` hook persists every user prompt via `sv-memory capture prompt`, and `SessionEnd` auto-closes the session with `sv-memory session end` (idempotent). The Claude Code lifecycle now mirrors the OpenCode plugin's model-agnostic behavior using the F2 CLI session commands.
...
- **Confidence column in `sv_graph_surprising_connections`**: the `SurprisingConnection` struct now carries a `Confidence` field populated from `edge.Confidence`, and the handler renders it in the markdown table between Edge Type and Surprise Score. Bridges with INFERRED/AMBIGUOUS confidence can now be prioritized for review.
- **Confidence filter on `sv_graph_query`**: new optional `confidence` parameter (`EXTRACTED`, `INFERRED`, `AMBIGUOUS`) filters edges after the BFS traversal. Nodes remain in the subgraph for context, but only matching edges are rendered. The Edge Confidence Breakdown reflects only filtered edges.
- **Graph Multi-Result Node Search (`sv_graph_search`)**: a new `SearchNodes` in `internal/graph/memory.go` discovers graph nodes matching a text pattern across id/label/path, ranked deterministically (exact path > exact label > substring; ties broken by degree, shortest id, then alphabetical). Unlike `FindNode` (single best match), this is the discovery path for agents that do not know the exact symbol name. The MCP tool `sv_graph_search` supports `query` (required), `limit` (default 10, max 50), and optional `node_type` filter (file, function, class, package, etc). Both `sv_graph_query` and `sv_graph_explain` now suggest `sv_graph_search(query=...)` when no node matches.
- **Graph Communities MCP Parity (`sv_graph_communities`)**: a new MCP tool listing the top communities in the dependency graph with auto-labels and member counts, or detailing a specific community's members. Uses `ExtractCommunities` (or `LeidenDetectCommunities` on the fly when metadata is missing) + `computeCommLabels`. Parameters: `top_n` (default 10, max 50), optional `community_id` to detail members. MCP parity for the existing `sv-memory graph communities` CLI command.
...
- **Intelligent Memory Health & Auto-Compaction (`CheckCompactionHealth` / `MaybeAutoCompact` / `sv-memory compact` / `sv_mem_compact`)**: new threshold-based auto-compaction and health monitoring in `internal/memory/compaction.go`. `CheckCompactionHealth` inspects multi-row topic key fragmentation; `MaybeAutoCompact` runs atomic incremental compaction when fragmented topics meet or exceed the threshold (default 3). The standalone CLI command `sv-memory compact` supports `--auto`, `--threshold`, and `--check` modes. The MCP tool `sv_mem_compact` gains `auto`, `threshold`, and `check_only` parameters. Opportunistic auto-compaction triggers silently at session end (`handleSessionEnd`), keeping storage lean and token-efficient across long agent workflows. Diagnostics (`RunDiagnostics`) and stats (`GetStats` / `sv_mem_stats`) now surface memory fragmentation health and compaction recommendations.
- **Structural Graph Diff (`sv-memory graph diff` / `sv_graph_diff` / `ComputeGraphDiff`)**: a new architectural difference engine in `internal/graph/diff.go` compares structural code elements (added/removed/modified symbols, imports, and calls) and calculates upstream blast radius impact between a Git base reference (e.g. `origin/HEAD`, `main`, `master`, `HEAD~1`) and the current working directory. The new CLI subcommand `sv-memory graph diff [base_ref] [--blast-radius]` and MCP tool `sv_graph_diff` provide both AI coding agents and human developers with immediate visibility into structural modifications and architectural blast radius risks prior to committing.
- **Agent-Agnostic OpenSpec Spec-Driven Decision Parity (`sv_update_spec` / Modular OpenSpec Mirror)**: comprehensive interoperability improvements aligning `sv-memory` with OpenSpec (Fission-AI/openspec) across all AI coding assistants (Antigravity, OpenCode, Claude Code, Cursor, Windsurf, Codex). Added the `sv_update_spec` MCP tool allowing agents to incrementally update task checklists (`- [x]`), refine technical design, proposal rationale, goal, affected paths, or delta requirements during the Apply phase with auto-calculated progress metrics (`FormatTaskProgress`). Extended `ImportChangeFromMarkdown` and `WriteSpecMirror` in `internal/memory/specs.go` to seamlessly read, parse, and export both single-file mirrors (`.sv-memory/specs/changes/<slug>.md`) and modular OpenSpec folder layouts (`openspec/changes/<slug>/` or `.sv-memory/specs/changes/<slug>/` containing `proposal.md`, `design.md`, `tasks.md`, `specs.md`). Enriched skill templates (`antigravity-skill.md`, `opencode-skill.md`), protocol definitions (`internal/protocol/protocol.go`), and `AGENTS.md` with complete OpenSpec lifecycle instructions.
...
- **Unambiguous & Idempotent Session End (`sv_mem_session_end` / `EndSession`)**: `EndSession` in `internal/memory/memory_session.go` now differentiates non-existent session IDs (`ErrSessionNotFound`) from already completed sessions (`ErrSessionAlreadyCompleted`). When called on an already completed session, `handleSessionEnd` returns an informative success message preserving existing summaries without triggering `isError: true` on MCP clients, eliminating false error recovery loops for AI agents.

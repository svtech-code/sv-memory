...
SV-Memory includes an active sanitization engine (located in [security.go](file:///Users/msandoval/.work/svtech/sv-memory/internal/security/security.go)) that uses regex patterns to redact sensitive credentials (such as API keys, JWTs, private keys, database connection strings) to `[REDACTED_SECRET]` before saving.

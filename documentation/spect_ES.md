...
- Verifica la existencia de `AGENTS.md`, `.cursorrules` o `.windsurfrules`:
  - Si existe alguno: Inyecta o actualiza el bloque `<!-- SV-MEMORY:START -->
...
- Registra las 42 herramientas MCP.
...
- **Fase 4 (Permisos MCP):** Lista las 42 herramientas MCP de sv-memory con descripciones y otorga las entradas de allow-list seleccionadas a las plataformas con allow-list elegidas previamente (Antigravity CLI, Claude Code).
...
- `list`: muestra las 42 herramientas MCP de sv-memory con descripciones legibles.
...
- **Antigravity CLI:** registra el servidor MCP, instala los hooks de `.agents/hooks.json`, inyecta `AGENTS.md` y concede el allow-list de 42 herramientas.
...
CREATE TABLE IF NOT EXISTS memories (
    ...
    deleted_at DATETIME,
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    change_id TEXT,             -- vincula una decisión confirmada a su change
...
-- Ciclo de vida de cambios (motor de decisiones spec-driven)
-- Cada propuesta recorre el ciclo de decisión:
-- 'draft' → 'proposed' → 'validated' → 'applied' (→ 'archived') | 'rejected'
CREATE TABLE IF NOT EXISTS changes (
    id TEXT PRIMARY KEY,
    project_id TEXT NOT NULL,
    slug TEXT NOT NULL,          -- kebab-case, UNIQUE por proyecto
    status TEXT NOT NULL DEFAULT 'draft',
    title TEXT NOT NULL,
    what TEXT,                   -- propuesta: por qué y qué cambia
    goal TEXT,
    where_path TEXT,             -- rutas de código afectadas (edges AFFECTS)
    design TEXT,                 -- enfoque técnico
    tasks TEXT,                  -- checklist de implementación
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME,
...
`sv-memory` registra **42 herramientas MCP** para agentes de IA:
...
Descubre nodos del grafo que coinciden con un patrón de texto en id/label/path. A diferencia de `sv_graph_explain` y `sv_graph_query` que resuelven un solo nodo exacto (vía `FindNode`), esta es la herramienta de descubrimiento: agentes que no conocen el nombre exacto del símbolo pueden listar todos los nodos de código coincidentes con sus métricas de red. Los resultados se ordenan de forma determinista (path exacto > label exacto > substring; desempates por grado, luego id más corto, luego alfabético). El filtro opcional `node_type` restringe las coincidencias a un tipo específico de nodo (file, function, class, package, etc).

- **Parámetros:**
  - `query` (string, requerido): Patrón de texto a buscar en id, label y path de los nodos.
  - `limit` (string, opcional): Número máximo de resultados (por defecto `10`, máximo `50`).
  - `node_type` (string, opcional): Filtro por tipo de nodo (file, function, class, package, etc).
- **Salida:** Tabla Markdown con columnas: `#`, `Node`, `Type`, `Path`, `Degree`, `Fan-In`, `Fan-Out`, `Community`. Incluye benchmark de tokens vs leer archivos raw.
- **Comportamiento:** Devuelve nil cuando no hay coincidencias (con sugerencias de fallback incluyendo `sv_graph_god_nodes` y `sv_graph_report`).

---

### 31. `sv_graph_communities`

Lista las principales comunidades del grafo de dependencias con auto-etiquetas y tamaños. Opcionalmente detalla los nodos miembros de una comunidad específica. Es el espejo MCP del comando CLI `sv-memory graph communities` (que usa `LeidenDetectCommunities` + `DetectCommunityLabels`).

- **Parámetros:**
  - `top_n` (string, opcional): Número de comunidades principales a listar (por defecto `10`, máximo `50`).
...
│   ├── mcp/                     # Servidor MCP + 42 handlers de herramientas; lee del caché LRU de internal/graph
...
**Extracción de aristas `calls` (precisión AST):** las aristas `calls` se producen por archivo prefiriendo el AST (tree-sitter o `go/parser` estándar para Go) con confianza `EXTRACTED` y una ubicación precisa `L<línea>:<columna>`, resolviendo cada sitio de llamada contra los nodos de función/clase del proyecto (mismo archivo primero, luego coincidencia única cross-file dentro del grupo de lenguaje). Los archivos cuyo lenguaje no tiene cobertura AST de llamadas (Lua, Markdown, shell, bloques script de Vue/Svelte/Astro) caen al heurístico de tokenize con confianza `INFERRED`. La ruta AST no captura identificadores dentro de strings o comentarios, eliminando una clase de falsos positivos que el heurístico produce. Esto mejora la precisión de `sv_graph_query`, `sv_graph_explain`, god nodes y la detección de comunidades en lenguajes con cobertura AST (Go, Python, JS/TS, Java, PHP, Ruby, Rust, CSS, HTML).

...
type Node struct {
	ID       string                 `json:"id"`
	Type     string                 `json:"type"` // 'file' | 'package' | 'function' | 'class' | 'section' | 'table' | 'view' | 'index' | 'type' | 'rationale'
...
type Edge struct {
	...
	RelationType   string `json:"relation_type"`             // 'imports' | 'calls' | 'depends_on' | 'contains' | 'rationale_for'
